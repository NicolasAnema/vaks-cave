...
**BOSS: Big Tikolosh — CAVE FM: LINE UP THE BEAT.** Vaks stays behind two battered turntables balanced on beer crates. At the end of the cave-mouth challenge, Spaza tells him to finish his Zamalek before they start. A short pixel-cartoon setup then lets that single beat breathe: the completed booth waits, an immersive face-and-bottle close-up follows Vaks through two serious gulps and a satisfied exhale, then he accidentally drops the bottle across the live record and admits `SE MISTAKE.` Beer spreads over the deck and Big Tikolosh's silent reaction makes the cave visibly shake. There are no competing setup gags or explanatory captions. Spaza's final `DALA VAKS.` opens the letterbox into the exact playable composition, where the deck check teaches Arrow and Space. Round 1 contains two unannounced but fully hittable VIBE pranks: each note nearly reaches the gold line, the record jerks backward, and the note physically rewinds before returning. After Round 2, a short authored load-shedding transition slows the record, kills the floodlight and music, reveals crowd phone torches, lets the dead generator cough out one spark and fail, then brings up Vaks's cat eyes and the gold target alone. Round 3 begins directly from that focused dark frame. Across the 10/13/14-note rounds, early or wrong presses remove two sync steps and drain VIBE. Only the final completed line-up converts the hostile crowd and makes Big Tikolosh's foot tap against his will, earning the respect paid off in "It's like the wind."
...
3. **Boss intro** (top of L3). A queue of Tikoloshes gathers to watch Vaks get moered. The familiar thief transforms into the Big Tikolosh and demands: "vibe with me." Spaza reveals the decks: "spin the decks, line up the beat."
4. **Boss resolution.** Continue from the final CAVE FM frame: Big Tikolosh's foot betrays him with two taps, then the music drops and wind ripples through the arena. After "It's like the wind," he delivers a darkly ceremonial monologue about the township dronkies who never escaped his cave, illustrated by abandoned Zamalek, vellie and car-key inserts. Vaks returns `VIBE WITH ME`; Big Tikolosh declares that he has earned respect amongst the Tikoloshes and calls `VIVO VICEROY!`, answered by the crowd's `VICEROY!!` He contracts into the familiar thief, returns the stolen money sack and sends Vaks toward dawn with the first sincere `RUN, MY BOSS.`
...
Boot -> title -> main menu -> loading screen -> cold open -> L1 -> shop -> L2 -> shop -> L3 -> boss intro -> CAVE FM setup cutscene -> deck check -> Round 1 with two vinyl rewinds -> Round 2 -> load-shedding transition -> Round 3 -> resolution -> chase begins -> L4 -> shop -> L5 -> shop -> L6 -> ending -> credits -> back to menu. Pause available in all gameplay. Level select, jukebox, gallery, settings, and credits reachable from the main menu. The camera system handles both orientations cleanly.
