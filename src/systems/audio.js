...
export const AudioManager = {
  ...
  setLevel(path, el, kind) {
    ...
    const level = this.vol(kind) * this.boost(kind);
  ...
  playFile(path, kind) {
    ...
    el.currentTime = 0;
    if (this.ctx && this.ctx.state === 'suspended') this.ctx.resume();
    el.play().catch(() => { /* pre-gesture autoplay block: text still shows */ });
    return el;
  },
  ...
  // vo/<stem>.opus covers every variant. One mouth: new note cuts old.
  voice(row, lineIdx) {
    ...
    if (!p) return;
    if (this.voiceEl && this.voiceEl !== this.cache.get(p)) this.voiceEl.pause();
    this.voicePath = p;
