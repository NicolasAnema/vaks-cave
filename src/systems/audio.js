...
export const AudioManager = {
  ...
  clearEl: null, clearPath: null,  // the LEVEL CLEAR screen fanfare (held so it can be cut on close)
  duck: false,   // true while a voice clip is playing; drops music gain
  restartNext: false, // when set, the next gameplay track starts from 0:00 (a lost-lives go-back asks for this)
  ...
  setLevel(path, el, kind, mul = 1, noDuck = false) {
    const g = this.gains.get(path);
    const duckMul = (kind === 'music' && this.duck && !noDuck) ? 0.55 : 1.0;
  ...
  playFile(path, kind, mul = 1, offset = 0) {
    const el = this.prepare(path, false);
    this.setLevel(path, el, kind, mul);
    // start at `offset` seconds in. If metadata isn't loaded yet the seek would
    // clamp to 0, so re-apply it once duration is known.
  ...
  voice(row, lineIdx, interrupt = false, offset = 0) {
    ...
    this.voicePath = p;
    const mul = (CONFIG.audio.voiceGain && CONFIG.audio.voiceGain[row.id]) || 1;
  ...
  startMusic(slotId) {
    ...
    // normally a (re)started track resumes from where the cached element left off;
    // on a lost-lives go-back we want it from the top. The silent loading slot
    // doesn't consume the flag, so it survives through to the level's track.
    if (this.restartNext && slotId !== 'loading') {
      this.restartNext = false;
      try { el.currentTime = 0; } catch (e) { /* not seekable yet: starts at 0 anyway */ }
    }
  ...
  // The LEVEL CLEAR screen fanfare (GTA "mission passed"). Plays once from the
  // top when the clear screen opens; the screen calls stopClear() the moment it's
  // dismissed. Runs at a steady music level (noDuck) so it never drowns the
  // "finished" voice note carrying over from the level — that voice punches
  // through on the louder voice channel.
  playClear() {
    const p = this.aliasPath(SFX_ALIASES.clear_fanfare);
    if (!p) return;
    const el = this.prepare(p, false);
    this.setLevel(p, el, 'music', 1, true);
    try { el.currentTime = 0; } catch (e) { /* not seekable yet: starts at 0 anyway */ }
    const resume = this.ctx && this.ctx.state === 'suspended' ? this.ctx.resume() : Promise.resolve();
    resume.then(() => el.play().catch(() => {}));
    this.clearEl = el; this.clearPath = p;
  },
  stopClear() {
    if (this.clearEl) {
      this.clearEl.pause();
      try { this.clearEl.currentTime = 0; } catch (e) { /* fine */ }
    }
    this.clearEl = null; this.clearPath = null;
  },

  // live re-apply when the settings sliders move (or mute toggles)
  applyVolumes() {
    if (this.musicEl && this.musicPath) this.setLevel(this.musicPath, this.musicEl, 'music');
    if (this.voiceEl && this.voicePath) this.setLevel(this.voicePath, this.voiceEl, 'voice');
    if (this.clearEl && this.clearPath) this.setLevel(this.clearPath, this.clearEl, 'music', 1, true);
  },
  ...
  // does a real uploaded track back this slot? (the jukebox hides silent slots)
  hasMusic(slotId) {
    const slot = MUSIC_SLOTS.find((m) => m.id === slotId);
    return !!(this.aliasPath(MUSIC_ALIASES[slotId]) || (slot && this.resolve(stemOf(slot.file))));
  },
};
